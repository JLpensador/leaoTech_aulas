...
const JWT_SENHA = process.env.JWT_SENHA || 'secret_finance_system'
...
app.get('/cliente', autenticarToken, autorizarTipo('cliente'), (req, res) => {
    res.json({
        mensagem: `Bem-vindo cliente ${req.usuario.nome}`,
        usuario: req.usuario
    })
})

app.get('/funcionario', autenticarToken, autorizarTipo('funcionario'), (req, res) => {
    res.json({
        mensagem: `Bem-vindo funcionário ${req.usuario.nome}`,
        usuario: req.usuario
    })
})
